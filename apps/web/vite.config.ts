...
const isPairDesktopBuild = process.env.T3CODE_DESKTOP_FLAVOR === "pair";
...
const configuredRelayUrl = isPairDesktopBuild ? "" : (repoEnv.VITE_T3CODE_RELAY_URL?.trim() ?? "");
const configuredClerkPublishableKey = isPairDesktopBuild
  ? ""
  : (repoEnv.VITE_CLERK_PUBLISHABLE_KEY?.trim() ?? "");
const configuredClerkJwtTemplate = isPairDesktopBuild
  ? ""
  : (repoEnv.VITE_CLERK_JWT_TEMPLATE?.trim() ?? "");
const configuredClerkCliOAuthClientId = isPairDesktopBuild
  ? ""
